...
def pretrain(
    ...
    if not args.skip_train:
        ...
        if args.compression_training:
            model = [
                redundancy_clean(model[0], args.deepspeed_config_dict, mpu)
            ]
...
def setup_model_and_optimizer(
    ...
    if args.kd or args.mos:
        ...
        print_rank_0(
            "***>>>>> Student model, global step:{}".format(
    ...
    if args.compression_training:
        ...
        model = [model]
        model = [
            init_compression(model[0].module, args.deepspeed_config_dict, mpu)
        ]
    ...
    if args.deepspeed:
        ...
        if isinstance(model, deepspeed.PipelineEngine):
            ...
            assert (
                model.grid.get_data_parallel_rank()
...
def training_log(
    ...
    if args.log_timers_to_tensorboard and (
        ...
        timers.write(
            timers_to_log, writer, iteration, normalizer=total_iterations
        )
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        if args.log_learning_rate_to_tensorboard:
            writer.add_scalar(
                "learning-rate/learning-rate", learning_rate, iteration
            )
            writer.add_scalar(
        ...
        if args.log_world_size_to_tensorboard:
            writer.add_scalar(
                "world-size/world-size", args.world_size, iteration
            )
            writer.add_scalar(
        ...
        if num_zeros_in_grad is not None:
            writer.add_scalar(
                "num-zeros/num-zeros", num_zeros_in_grad, iteration
            )
            writer.add_scalar(
        ...
        if params_norm is not None:
            writer.add_scalar(
                "params-norm/params-norm", params_norm, iteration
            )
            writer.add_scalar(
    ...
    if iteration % args.tensorboard_log_interval == 0:
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if writer and is_last_rank():
                ...
                writer.add_scalar(
                    "optimizer/weight_l2", opt_stats[3] ** 0.5, iteration
                )
                writer.add_scalar(
                    "optimizer/variance_l1", opt_stats[4], iteration
                )
                writer.add_scalar(
                    "optimizer/variance_sqrt_l1", opt_stats[5], iteration
                )
                writer.add_scalar(
                    "optimizer/momentum_l1", opt_stats[6], iteration
                )
                writer.add_scalar(
                    "optimizer/weight_l1", opt_stats[7], iteration
                )
                writer.add_scalar(
                    "optimizer/variance_abs_max", opt_stats_2[0], iteration
                )
                writer.add_scalar(
                    "optimizer/variance_sqrt_abs_max",
                    opt_stats_2[1],
                    iteration,
                )
                writer.add_scalar(
                ...
                writer.add_scalar(
                    "optimizer/weight_abs_max", opt_stats_2[3], iteration
                )
    ...
    if iteration % args.log_interval == 0:
        ...
        if wandb is not None and getattr(wandb, "run", None) is not None:
            ...
            if loss_dict is not None:
                wandb_metrics |= {f"loss/{k}": v for k, v in loss_dict.items()}
...
def train(
    ...
    while iteration < args.train_iters and (
        ...
        if iteration == 1 and args.enable_tensor_cache:
            ...
            if args.tensor_cache_in_memory_adapter:
                # TODO: In the first iteration, merely measure the offloaded data but discard it, i.e., not actually offload the data. In this way we can avoid the high memory use in the first iteration due to slow offloading.
                if isinstance(tensor_cache, PTC.PipelineTensorCache):
                    tensor_caches = tensor_cache.tensor_caches
                else:
                    tensor_caches = [tensor_cache]
                ...
                if isinstance(tc_.offloader.engine.adapter, tcadapters.RevolverIOAdapter):
                    ...
                    tc_.offloader.engine.adapter.instantiate_host_pinned_memory_allocator(peak_memory)
        elif iteration > 1:
        ...
        if (
            ...
            if hasattr(args, "data_efficiency_curriculum_learning_numel"):
                ...
                args.consumed_train_tokens += (
                    new_samples * args.actual_seq_length
                )
        else:
...
def evaluate_and_print_results(
    ...
    for key in total_loss_dict:
        string += "{} value: {:.6E} | ".format(
            key, total_loss_dict[key].item()
        )
