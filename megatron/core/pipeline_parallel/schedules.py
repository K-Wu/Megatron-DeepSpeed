...
from flashtrain.tensor_cache.pipeline_tensor_cache import (
    PipelineTensorCache,
    Stage,
)
...
def forward_backward_no_pipelining(
    *,
    ...
    tensor_cache: Optional[PipelineTensorCache] = None,
):
    ...
    args = get_args()
    if args.profile_first_iter or args.profile_first_iter_longer or args.profile_unlimited:        
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    no_sync_func = config.no_sync_func
    if no_sync_func is None and all(
        isinstance(chunk, torchDDP) for chunk in model
    ):
    ...
    if num_microbatches % pipeline_parallel_size != 0:
        msg = (
            f"number of microbatches ({num_microbatches}) is not divisible by "
        )
    ...
    if (
        decoder_seq_length is not None
        and decoder_seq_length != tensor_shape[0]
    ):
        raise RuntimeError(
    ...
    def forward_step_helper(microbatch_id, checkpoint_activations_microbatch):
        ...
        if config.param_sync_func is not None:
            ...
            if (
                ...
                and is_first_microbatch_for_model_chunk(
                    param_sync_microbatch_id
                )
            ):
                param_sync_chunk_id = (
                    get_model_chunk_id(param_sync_microbatch_id, forward=True)
                    + 1
                )
    ...
    def backward_step_helper(microbatch_id):
        ...
        if config.grad_sync_func is not None:
            ...
            if (
                grad_sync_microbatch_id >= 0
                and is_last_microbatch_for_model_chunk(grad_sync_microbatch_id)
            ):
    ...
    for k in range(num_warmup_microbatches):
        ...
        output_tensor = forward_step_helper(
            k, checkpoint_activations_microbatch
        )
...
def send_backward(input_tensor_grads, tensor_shapes, config):
    ...
    for input_tensor_grad, tensor_shape in zip(
        input_tensor_grads, tensor_shapes
    ):
        if tensor_shape is None:
            continue
